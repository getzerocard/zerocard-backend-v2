...
import { getRepository } from 'typeorm';  // Or use the appropriate import for EntityManager

@Injectable()
export class SaveFiatWithdrawalHandler {
    private readonly logger = new Logger(SaveFiatWithdrawalHandler.name);

    async saveFiatWithdrawal(
        ...
        this.logger.log(`Fiat withdrawal transaction saved for order ${offrampResponse.orderId} for user ${user.userId} with status ${status}`);
    }
}
